@if ($paginator->hasPages())
    <div class="flex items-center justify-center w-full">
        ...
        @else
            <a
                class="rounded-l rounded-sm border-t border-b border-l border-brand-light px-3 py-2 text-brand-dark hover:bg-gray-700 no-underline"
        ...
        @foreach ($elements as $element)
            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    ...
                    @else
                        <a class="border-t border-b border-l border-brand-light px-3 py-2 hover:bg-gray-700 text-brand-dark hover:text-gray-100 no-underline" href="{{ $url }}">{{ $page }}</a>
                    @endif
                @endforeach
            @endif
        @endforeach
